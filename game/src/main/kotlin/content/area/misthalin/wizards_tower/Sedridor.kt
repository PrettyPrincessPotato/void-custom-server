...
class Sedridor : Script {
        ...
        npcOperate("Talk-to", "sedridor") { (target) ->
            when (quest("rune_mysteries")) {
                "unstarted" -> {
                    ...
                    player<Idle>("I'm just looking around.")
                    npc<Confused>("Well, take care adventurer. You stand on the ruins of the old destroyed Wizards' Tower. Strange and powerful magicks lurk here.")
                }
                "started" -> started()
    ...
    suspend fun Player.started() {
        ...
        player<Idle>("The Duke of Lumbridge sent me to find him. I have this talisman he found. He said Sedridor would be interested in it.")
        npc<Idle>("Did he now? Well hand it over then, and we'll see what all the hubbub is about.")
        choice {
            option("Okay, here you are.") {
                okHere()
            }
            option<Confused>("No, I'll only give it to Sedridor.") {
                npc<Happy>("Well good news, for I am Sedridor! Now, hand it over and let me have a proper look at it, hmm?")
                choice {
                    option("Okay, here you are.") {
                        okHere()
                    }
                    option<Confused>("No, I don't think you are Sedridor.") {
                        ...
                        player<Shock>("You're right! How did you know that?")
    ...
    suspend fun Player.okHere() {
        ...
        if (inventory.contains("talisman_rune_mysteries")) {
            ...
            player<Confused>("First tower? Forgotten essence mine? What are you on about?")
            npc<Happy>("Ah, my apologies, adventurer. Allow me to fill you in.")
            choice {
                option<Idle>("Go ahead.") {
                    ...
                    npc<Disheartened>("When the tower burnt down, much was lost, including an important incantation. A spell that could be used to teleport to a hidden essence mine.")
                    player<Quiz>("The essence mine you mentioned earlier, I assume?")
                    npc<Idle>("Precicely. Rune essence is the material used to make runes, but it is incredibly rare. That essence mine was the only place it could be found that our order knew of.")
                    npc<Disheartened>("Since the incantation was lost, we have struggled to maintain our stocks of rune essence.")
                    npc<Idle>("There are seemingly those out there that still know where to find some, but while they have been willing to sell essence to us, they have refused to share knowledge on how to find it ourselves.")
                ...
                option<Idle>("Actually, I'm not interested.") {
                    npc<Disheartened>("Oh... Well I guess the short of it is that this talisman could be key to helping us rediscover an important teleportation incantation.")
                    npc<Idle>("With it, we'll be able to access a hidden essence mine, our lost source of rune essence.")
                    discovery()
                }
            }
        } else {
            ...
            player<Neutral>("...")
            npc<Confused>("Well?")
            player<Confused>("I don't seem to have it with me.")
            npc<Confused>("Hmm? You are a very odd person. Come back again when you have it.")
        }
    }
    ...
    suspend fun Player.checkPackageDelivered() {
        ...
        if (ownsItem("research_package_rune_mysteries")) {
            player<Idle>("Not yet.")
            npc<Idle>("He runs a rune shop in the south east of Varrock. Please deliver it to him soon.")
        } else {
    ...
    suspend fun Player.checkResearchDelivered() {
        ...
        if (carriesItem("research_notes_rune_mysteries")) {
            ...
            player<Confused>("Err, you're not going to believe this...")
            npc<Confused>("What?")
            player<Confused>("I don't have them.")
            npc<Confused>("Right... You're rather careless aren't you. I suggest you go and speak to Aubury once more. With luck he will have made copies.")
        }
    }

    fun ChoiceOption.imBusy(): Unit = option<Idle>("No, I'm busy.") {
        npc<Idle>("As you wish adventurer. I will continue to study this talisman you have brought me. Return here if you find yourself with some spare time to help me.")
    }

    fun ChoiceOption.yesCertainly(): Unit = option<Idle>("Yes, certainly.") {
    ...
    fun ChoiceOption.oldWizardsTower(target: NPC): Unit = option<Quiz>("Could you tell me about the old Wizards' Tower?") {
        ...
        npc<Disheartened>("Alas, that openness is what ultimately led to disaster. The wizards who served Zamorak, the evil god of chaos, tried to claim our magical discoveries in his name.")
        npc<Disheartened>("They failed, but in retaliation, they burnt the entire tower to the ground. Years of work was destroyed.")
        npc<Idle>("The tower was soon rebuilt of course, but even now we are still trying to regain knowledge that was lost.")
        npc<Idle>("That's why I spend my time down here, in fact. This basement is all that is left of the old tower, and I believe there are still some secrets to discover here.")
